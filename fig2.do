...
********************************************************************************
*
* Figure 2: Effects of four temperature measures on mortality risk of older adults
*
********************************************************************************
...
use "$data/main.dta", clear
	...
	*regression
	...
	parmest , saving($figure/fig2/fig2_all.dta, replace)
	...
	* figure
		...
		*adjust numerical scheme
		...
		graph twoway (rcap min95 max95 y if y == 1, horizontal lcolor(gold*1.2) lwidth(0.4)) ///
		 ...
		 graph export "$figure/fig2/fig2_1.png", replace width(3000)
	...
	reghdfe death c.mean_maxhi##c.mean_maxhi c.sd_meanhi c.TN95_adthi c.TD95_adthi c.annual_pre_365 $control , absorb(prov#year prov#month) cluster(gbcode)
		...
		* Calculated marginal effect
		...
		graph twoway hist mean_maxhi, width(0.5) percent color(eltblue*0.8) yaxis(2)        ///
		 ...
		 graph save "$figure/fig2/fig2_2", replace
		 graph export "$figure/fig2/fig2_2.png", replace width(3000)
		...
		* Calculated marginal effect
		...
		graph twoway hist TD95_adthi, width(0.5) percent color(eltblue*0.8) yaxis(2)        ///
			...
			||   ,    ///
				 ...
				 ysize(18) xsize(20) ///
				 scheme(s1mono) graphregion(fcolor(white) ilcolor(white) lcolor(white))
		 graph save "$figure/fig2/fig2_3", replace
		 graph export "$figure/fig2/fig2_3.png", replace width(3000)
		...
		* Calculated marginal effect
		...
		graph twoway hist mean_maxhi, width(0.5) percent color(eltblue*0.8) yaxis(2)        ///
			...
			||   ,    ///
				 ...
				 ysize(18) xsize(20) ///
				 scheme(s1mono) graphregion(fcolor(white) ilcolor(white) lcolor(white))
		 graph save "$figure/fig2/fig2_4", replace
		 graph export "$figure/fig2/fig2_4.png", replace width(3000)
